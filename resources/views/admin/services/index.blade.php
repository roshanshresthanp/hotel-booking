  ...
  <div class="col-md-12">
    ...
    <div class="card">
      <div class="px-4 py-3 cus-center justify-content-between" style="border-bottom: 1px solid rgb(218, 218, 218);">
        <h3 class="card-title">{{__('Available')}} {{__('Services')}}</h3>
        <div class="text-right">
          <a href="{{route('admin.services.create')}}" class="btn btn-sm btn-info"><i class="fas fa-plus mr-2"></i>{{__('Add')}} {{__('Service')}}</a>
        </div>
      </div>
      <!-- /.card-header -->
      <div class="card-body">
            ...
            @foreach ($service as $item)
              <tr>
                <td>{{$loop->iteration}}</td>
                  <td>{{$item->title}}</td>
                  <td>{!!str_limit($item->description,400)!!}</td>

                  <td>@if ($item->status == '1') <span class="badge badge-success">Active</span> @else <span class="badge badge-danger">Inactive</span> @endif </span></td>
                  
                  {{-- <td><img src="{{asset('/storage/service')}}/{{$item->image}}" height="60" width="80" alt="icon"> </td> --}}

                <td class="cus-center justify-content-around">

                  <form action="{{route('admin.services.destroy',$item->id)}}" method="POST">
                    <a href="{{route('admin.services.edit',$item->id)}}" title="Edit" class="btn btn-sm btn-success"><i class="fas fa-edit"></i></a>
                    @csrf
                    @method('DELETE')
