  ...
  <div class="col-md-12">
    ...
    <div class="card">
      <div class="px-4 py-3 cus-center justify-content-between" style="border-bottom: 1px solid rgb(218, 218, 218);">
        <h3 class="card-title">{{__('Available')}} {{__('Explore')}}</h3>
        <div class="text-right">
          <a href="{{route('admin.about-explore.create')}}" class="btn btn-sm btn-info"><i class="fas fa-plus mr-2"></i>{{__('Add')}} {{__('About explore')}}</a>
        </div>
      </div>
      <!-- /.card-header -->
      <div class="card-body">
            ...
            @foreach ($explore as $about)
              <tr>
                <td>{{$loop->iteration}}</td>
                  ...
                  <td><img src="{{asset('/storage/aboutexplore')}}/{{$about->image}}" height="60" width="80" alt="icon"> </td>

                <td class="cus-center justify-content-around">

                  <form action="{{route('admin.about-explore.destroy',$about->id)}}" method="POST">
                    <a href="{{route('admin.about-explore.edit',$about->id)}}" title="Edit" class="btn btn-sm btn-success"><i class="fas fa-edit"></i></a>
                    @csrf
                    @method('DELETE')
