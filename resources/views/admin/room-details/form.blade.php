  ...
  <div class="col-md-8">
    <div class="card d-flex card-info">
      <div class="card-header">
        <h3 class="card-title">@if(isset($detail)) {{__('Update')}}@else {{__('Add')}} {{__('Feature')}} @endif</h3>
      </div>
      <!-- /.card-header -->
            ...
            <div class="row">
              ...
              <div class="col-md-12">
                  <div class="form-group">
                      <label>{{__('Detail Name')}}</label>
                      <input type="text" class="form-control" value="{{old('name',isset($detail)?$detail->name:'') }}" placeholder="Feature name" name="name" >
                  </div>
              </div>
            </div>
              ...
              <label>{{__('Status')}}</label>
                <div class="form-check">
                  ...
                  {{-- <input class="form-check-input" type="checkbox" data-toggle="toggle" data-on="Yes" data-off="No" value="1" id="flash"
                      name="status"> --}}
                      <input class="form-check-input" @if(isset($detail))  @if($detail->status == '1')
                ...
                {{-- <div class="col-md-12" onMouseOver="this.style.border='1px solid black'" onMouseOut="this.style.border='0'">
                  ...
                  <input type="file" name="icon" class="dropify" @if(isset($detail))  data-default-file="{{ asset('storage/icon/'.$detail->icon) }}" @endif data-allowed-file-extensions="jpg jpeg png webp" />
