  ...
  <div class="col-md-12">
    ...
    <div class="card">
      <div class="px-4 py-3 cus-center justify-content-between" style="border-bottom: 1px solid rgb(218, 218, 218);">
        <h3 class="card-title">{{__('Available')}} {{__('Banners')}}</h3>
        <div class="text-right">
          <a href="{{route('admin.banners.create')}}" class="btn btn-sm btn-info"><i class="fas fa-plus mr-2"></i>{{__('Add')}} {{__('Banner')}}</a>
        </div>
      </div>
      <!-- /.card-header -->
      <div class="card-body">
            ...
            @foreach ($banners as $banner)
              <tr>
                <td>{{$loop->iteration}}</td>
                  ...
                  <td>@if ($banner->status == '1') <span class="badge badge-success">Active</span> @else <span class="badge badge-danger">Inactive</span> @endif </span></td>
                  
                  <td><img src="{{asset('/storage/banner')}}/{{$banner->image}}" height="60" width="80" alt="icon"> </td>

                <td class="cus-center justify-content-around">

                  <form action="{{route('admin.banners.destroy',$banner->id)}}" method="POST">
                    <a href="{{route('admin.banners.edit',$banner->id)}}" title="Edit" class="btn btn-sm btn-success"><i class="fas fa-edit"></i></a>
                    @csrf
                    @method('DELETE')
